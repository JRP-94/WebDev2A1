...
const CalculateNetTotal = (total, budget) =>
    RoundToTwoDecimalPlaces(budget - total);

// Calculates the outstanding balance based on the total and the initial savings
// returns the outstanding balance
const CalculateOutstandingBalance = (total, savings) =>
    RoundToTwoDecimalPlaces(total - savings);
...
function CalculateResult(cols, rows) {
    ...
    for (let c = 1; c <= cols; c++) {
        ...
        subtotalField.value =
            RoundToTwoDecimalPlaces(subtotal).toFixed(NUM_DECIMAL_PLACES);
        sum += subtotal;
    }
